...
static const char *family_name(enum ryzen_family fam)
{
	...
	case FAM_PHOENIX2: return "Phoenix Point 2";
...
int main(int argc, const char **argv)
{
	...
	struct argparse_option options[] = {
		...
		OPT_U32('t', "tctl-temp", &tctl_temp, "Tctl Temperature Limit (degree C)"),
		...
		OPT_BOOLEAN('s', "power-saving", &power_saving, "Hidden options to improve power efficiency (is set when AC unplugged): behavior depends on CPU generation, Device and Manufacture"),
		OPT_BOOLEAN('p', "max-performance", &max_performance, "Hidden options to improve performance (is set when AC plugged in): behavior depends on CPU generation, Device and Manufacture"),
		OPT_U32('l', "power_limit", &power_limit, "Set system power limit (W) (value between 8 and 54)"),
